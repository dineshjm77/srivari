                ...
                <div class="row align-items-center mb-4">
                    <div class="col">
                        ...
                        <small class="text-muted">
                            Track all EMI collections with detailed analysis
                            <?php if ($current_user_role == 'staff'): ?>
                                <span class="badge bg-info ms-2">Staff View (Your collections only)</span>
                            <?php elseif ($current_user_role == 'accountant'): ?>
                                <span class="badge bg-warning ms-2">Accountant View</span>
                            <?php elseif ($current_user_role == 'admin'): ?>
                                <span class="badge bg-success ms-2">Administrator View</span>
                            <?php endif; ?>
                        </small>
                    </div>
                    <div class="col-auto">
                    ...
                    <div class="card-body">
                        <form method="GET" action="" class="row g-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select class="form-control" name="collector_id" <?php echo ($current_user_role == 'staff') ? 'disabled' : ''; ?>>
                                    <?php if ($current_user_role == 'admin' || $current_user_role == 'accountant'): ?>
                                    ...
                                    <?php elseif ($current_user_role == 'staff'): ?>
                                        <?php foreach ($collectors as $collector): ?>
                                            <option value="<?= $collector['id']; ?>" selected>
                                                <?= htmlspecialchars($collector['full_name']); ?> (You)
                                            </option>
                                        <?php endforeach; ?>
                                        <input type="hidden" name="collector_id" value="<?= $current_user_id; ?>">
                                    <?php else: ?>
                                        <option value="<?= $current_user_id; ?>" selected>
                                            <?= htmlspecialchars($current_user_name); ?>
                                        </option>
                                    <?php endif; ?>
                                </select>
                                <?php if ($current_user_role == 'staff'): ?>
                                    <small class="text-muted">You can only view your own collections</small>
                                <?php endif; ?>
                            </div>
